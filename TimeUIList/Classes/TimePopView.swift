...
public class TimePopView: BasePopVC {
    ...
    public  func getLunarString()->String{
        if let lunar = getLunar() {
         ...
         if lunar.isleap {
            return "\(lunarYear)年闰\(lunarMonths)\(lunarDays)\(DateUtil.chineseTimedate[rows.3])"
         }
         return "\(lunarYear)年\(lunarMonths)\(lunarDays)\(DateUtil.chineseTimedate[rows.3])"
         }
         return ""
     }
     ...
     public func getSolar()->String{
         ...
         let solarDay = Int(solar.solarDay)
            return "\(solarYear)年\(solarMonth)月\(solarDay)日 \(DateUtil.getDayString(rows.3))时"
         }
        return ""
     }
...
extension TimePopView : UIPickerViewDelegate,UIPickerViewDataSource
{
    ...
    public func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        if component == 1 {
            ...
            label.text =  DateUtil.chineseTimedate[row] + " \(row)点"
        }
        label.textAlignment = .center
        return label
    }
